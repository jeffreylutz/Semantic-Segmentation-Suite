...
if args.model == "FC-DenseNet56" or args.model == "FC-DenseNet67" or args.model == "FC-DenseNet103":
    ...
    network, init_fn = build_pspnet(input, label_size=[args.crop_height, args.crop_width], preset_model=args.model,
...
if args.mode == "train":
    ...
    for epoch in range(0, args.num_epochs):
        ...
        for i in range(num_iters):
            ...
            # Do the training
            # tf.summary.summaries.add(tf.summary.image('image_tensor,max_images-15,name='images'))
        ...
        for ind in val_indices:
            ...
            accuracy, class_accuracies, prec, rec, f1, iou = utils.evaluate_segmentation(pred=output_image, gt=gt,
                                                                                         num_classes=num_classes)
    ...
    for ind in range(len(test_input_names)):
        ...
        input_image = np.expand_dims(np.float32(load_image(test_input_names[ind])[:args.crop_height, :args.crop_width]),
                                     axis=0) / 255.0
    ...
    input_image = np.expand_dims(np.float32(load_image(test_input_names[args.ind])[:args.crop_height, :args.crop_width]),
                                 axis=0) / 255.0
    ...
    output_image = helpers.reverse_one_hot(output_image)
    out_vis_image = helpers.colour_code_segmentation(output_image, class_dict)
    cv2.imwrite("%s/%s_pred.png" % ("Test", args.file_name), np.uint8(out_vis_image))
else:
